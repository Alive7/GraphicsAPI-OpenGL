...
Triangle::Triangle(float s1, float s2, float s3, Triangle_type tri) {
    ...
    else if (tri == Triangle_type::SSA) vertices = createTriangle_SAS(s1, s2, s3);
    else if (tri == Triangle_type::SAA) vertices = createTriangle_ASA(s1, s2, s3);
...
Simplex::Simplex(float l) {
    vertices = createCone(3,1,l);
}

void Simplex::generateNormals(void) {};
...
/*
std::vector<float> createSimplex(float l) {
    float h = l * sqrt(3) / 2;
    std::vector<float> vs(createTriangle_SSS(l, l, l));
    vs.push_back(0);
    vs.push_back(0);
    vs.push_back(h);
    return vs;
